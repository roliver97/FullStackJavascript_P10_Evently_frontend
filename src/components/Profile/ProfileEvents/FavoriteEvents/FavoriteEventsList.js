...
export const renderFavoriteEventsList = (container, context) => {
  const { userProfile, isSelf, isAdmin, loggedUser } = context

  const userSortedFavoriteEvents = sortEventsByDate(userProfile.favorites)

  if (userSortedFavoriteEvents.length === 0) {
    container.innerHTML = ` <p class="profile-subtitle">
         ${isSelf ? 'Here you will find all your favorite events' : isAdmin ? `Here you will find all the events ${userProfile.username} liked` : ''}
      </p>
      `
  }
  ...
  userSortedFavoriteEvents.forEach((event) => {
