...
export const renderCreatedEventsList = (container, context) => {
  const { userProfile, isSelf, isAdmin, loggedUser } = context
  const userSortedEvents = sortEventsByDate(userProfile.createdEvents)

  if (userSortedEvents.length === 0) {
    container.innerHTML = ` <p class="profile-subtitle">
         ${isSelf ? 'Here you will find all your own events' : isAdmin ? `Here you will find all the events posted by ${userProfile.username}` : ''}
      </p>
      `
  }
  ...
  userSortedEvents.forEach((event) => {
